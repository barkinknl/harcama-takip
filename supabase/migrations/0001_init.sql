-- ============================================================
-- Harcama Takip — başlangıç şeması
...
-- ============================================================
-- profiles
-- auth.users'a 1-1 bağlı kullanıcı profili.
-- ============================================================
...
-- ============================================================
-- other_earnings
-- (Excel'deki "Other Earnings" tablosu)
-- ============================================================
create table if not exists public.other_earnings (
  id uuid primary key default uuid_generate_v4(),
  user_id uuid not null references auth.users(id) on delete cascade,
  amount numeric(14, 2) not null check (amount >= 0),
  date date not null,
...
create index if not exists other_earnings_user_month_idx
  on public.other_earnings(user_id, month_key);

alter table public.other_earnings enable row level security;

drop policy if exists "other_earnings_select_own" on public.other_earnings;
create policy "other_earnings_select_own" on public.other_earnings
  for select using (auth.uid() = user_id);

drop policy if exists "other_earnings_insert_own" on public.other_earnings;
create policy "other_earnings_insert_own" on public.other_earnings
  for insert with check (auth.uid() = user_id);

drop policy if exists "other_earnings_update_own" on public.other_earnings;
create policy "other_earnings_update_own" on public.other_earnings
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "other_earnings_delete_own" on public.other_earnings;
create policy "other_earnings_delete_own" on public.other_earnings
  for delete using (auth.uid() = user_id);

-- ============================================================
-- monthly_settings
-- Excel'deki üst panel: INCOME, LAST MONTH'S DEBIT, NEXT PAY
-- ============================================================
...
create index if not exists monthly_settings_user_month_idx
  on public.monthly_settings(user_id, month_key);
...
drop policy if exists "monthly_settings_delete_own" on public.monthly_settings;
...
drop trigger if exists monthly_settings_set_updated_at on public.monthly_settings;
create trigger monthly_settings_set_updated_at
  before update on public.monthly_settings
  for each row execute function public.set_updated_at();

-- ============================================================
-- Yeni kullanıcı: profil + default kategoriler
-- ============================================================
...
-- Sonraki ayın "last_month_debit" değerini önceki aydan hesaplar.
-- Manuel veya cron olarak çağrılabilir.
-- ============================================================
